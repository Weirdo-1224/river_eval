...
- **RIVER Retro-Memory API baseline**（第一阶段）
  - 模型能看到问题时间点之前的全部视频内容
  - 支持长短记忆帧策略（`river_long_short`）
  - 多选题，计算准确率

- **RIVER Live-Perception**（第二阶段）
  - 模型只能看到 question_time 附近的一个短时窗口（默认前 8 秒）
  - 模拟"当前时间点提问"的在线交互场景
  - 复用 Offline Runner，新增 `live_perception_window` 帧策略

- **RIVER Pro-Response-Instant**（第二阶段）
  - 模型主动监测视频流，在目标事件发生时触发报警
  - 使用 **Online Runner**（`online_run_eval.py`）逐滑动窗口调用模型
  - 评分指标：命中率、精确率、误报率、响应延迟、F1

- **RIVER Pro-Response-Streaming**（第二阶段）
  - 多轮开放式问答，对话上下文随轮次传递
  - Dataset 层面将多轮记录扁平化为独立 Sample
  - 支持关键词/重叠率开放式答案匹配评分
...
│       ├── river_live_perception_qwen.yaml
│       ├── river_pro_response_instant_qwen.yaml
│       └── river_pro_response_streaming_qwen.yaml
├── annotations/
│   ├── Retro-Memory.json
...
**Offline 任务**（Retro-Memory / Live-Perception / Streaming）：
...
**Online 任务**（Pro-Response-Instant）：

```bash
python -m benchmark.runners.online_run_eval \
  --config configs/experiments/river_pro_response_instant_qwen.yaml \
  --max-samples 1
```
